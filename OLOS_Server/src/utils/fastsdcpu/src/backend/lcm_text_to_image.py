...
class LCMTextToImage:
    ...
    def _validate_mode(
        ...
        if not self._is_valid_mode(modes):
            raise ValueError(
                "Invalid mode,delete configs/settings.yaml and retry!")
    ...
    def init(
        ...
        if (
            ...
            if self.use_openvino and is_openvino_device():
                ...
                controlnet_args = load_controlnet_adapters(
                ...
                if tomesd and token_merging > 0.001:
                    ...
                    tomesd.apply_patch(
                        self.img_to_img_pipeline, ratio=token_merging)
            ...
            self.previous_task_type = lcm_diffusion_setting.diffusion_task
            self.previous_lora = lcm_diffusion_setting.lora.model_copy(
                deep=True)
    ...
    def generate(
        ...
        if (
            ...
            img_to_img_inference_steps = ceil(
                1 / lcm_diffusion_setting.strength)
            print(
        ...
        if lcm_diffusion_setting.use_seed:
            ...
            seeds = [random.randint(0, 999999999) for i in range(
                lcm_diffusion_setting.number_of_images)]
