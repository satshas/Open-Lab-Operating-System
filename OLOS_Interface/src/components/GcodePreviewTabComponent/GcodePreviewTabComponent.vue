<template>
  <div class="column q-pa-lg">
    <div v-if="isFileOpen" class="column fit q-gutter-y-md">
      <main-buttons
        v-if="config?.machine_type !== Constants.MACHINE_TYPE.CNC"
      />
      <graph-component />
      <cnc-camera-stream
        v-if="
    ...
    <div v-else class="row message-text flex-center" style="height: 100vh">
...
onMounted(async () => {
  config.value = await configurationSettings();
  if (config.value.machine_type === Constants.MACHINE_TYPE.VINYL_CUTTER) {
    scrollToBottom();
  }
});
</script>
