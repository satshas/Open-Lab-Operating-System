...
const previewerTools = (() => {
  ...
  const maxDistance = Math.max(platformWidth * 10, platformHeight * 10, 1000);
  ...
  camera.position.set(
    0,
    (500 * Math.max(platformWidth, platformHeight)) /
      Math.min(platformWidth, platformHeight),
    0
  );
  ...
  renderer.setSize(platformWidth * 2, platformHeight * 2, true);
...
export const draw3DGcodeLinesOnGraph = (
  ...
  //   positionsArray[i] -= platformWidth / 2; // x
  //   positionsArray[i + 2] += platformHeight / 2; // z
  // }
