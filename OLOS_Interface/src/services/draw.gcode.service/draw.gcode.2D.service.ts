...
const previewerTools = (() => {
  ...
  labelRenderer.domElement.id = 'threejs-canvas';
...
export const draw2DGcodeLinesOnGraph = (
  ...
  while (index--) {
    ...
    opacitiesArray[baseOpacityIndex + 1] = opacity;
  }
...
export const setGraphHeightSize = (maxJobHeight: number) => {
  const offset = maxJobHeight >= 100 ? 100 : maxJobHeight;
  previewerTools.jobLayerElements.group.position.z =
    -platformHeight / 2 + (maxJobHeight + offset) / 2;

  previewerTools.camera.top = (maxJobHeight + offset) / 2;
  previewerTools.camera.bottom = -(maxJobHeight + offset) / 2 + 8;
  previewerTools.camera.updateProjectionMatrix();

  previewerTools.renderer.setSize(platformWidth, maxJobHeight + offset, true);
  previewerTools.labelRenderer.setSize(platformWidth, maxJobHeight + offset);
  previewerTools.gridHelper.clear();
  previewerTools.gridHelper = createCustomGrid(
    platformWidth,
    maxJobHeight + offset,
    graphStepSize
  );
...
const constrainBoxPosition = () => {
  if (previewerTools.jobLayerElements.group && initialBBox) {
    ...
    const minZ = -platformHeight / 2 - initialBBox.min.z;
    const maxZ = platformHeight / 2 - 8 - initialBBox.max.z;
