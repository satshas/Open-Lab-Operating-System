...
export const useGcodePreviewStore = defineStore('gcodePreview', {
  state: () => ({
    graphContainer: document.createElement('div') as HTMLDivElement,
  ...
  actions: {
    async createGcodePreviewerGraph(fileContent: string) {
      ...
      // this.resizeThreeJSCanvasToFitPageDimensions();
    },
    async create2DGraph(gcode: string) {
      ...
      /* change the height of the graph based on the job end point for only vinyl cutter machines
        because vinyl cutter y axis is infinite
       */
      if (config.machine_type === Constants.MACHINE_TYPE.VINYL_CUTTER) {
        setGraphHeightSize(this.graphSettings.endPoint.y);
      }

      await this.addGraphTools();
    },
        ...
        if (this.previewWebWorker) {
          this.previewWebWorker.addEventListener('message', async (e) => {
            ...
            if (done) {
              ...
              if (startPoint) this.graphSettings.startPoint = startPoint;
              if (endPoint) this.graphSettings.endPoint = endPoint;
    ...
    addGraphElementToGraphContainer() {
      if (this.graphContainer) {
        this.graphContainer.append(this.graphElement);
        this.resizeThreeJSCanvasToFitPageDimensions();
      }
    },
    ...
    async addGraphTools() {
      ...
      if (this.graphSettings.showGrid) {
        this.addGridTo2DGraph();
      }
      if (
        this.graphSettings.showCamera &&
        config.machine_type === Constants.MACHINE_TYPE.LASER_CUTTER
      ) {
        this.addCameraFrameTo2DGraph();
      }
      if (
        this.graphSettings.showLabels &&
        config.machine_type === Constants.MACHINE_TYPE.LASER_CUTTER
      ) {
        this.addLabelsTo2DGraph();
      }
    },
    addGridTo2DGraph() {
      add2DGrid();
    ...
    resizeThreeJSCanvasToFitPageDimensions() {
      // get three js canvas element
      const threeJSCanvasElements =
        this.graphElement.querySelectorAll('#threejs-canvas');
      if (threeJSCanvasElements.length) {
        threeJSCanvasElements.forEach((element) => {
          ...
          if (this.graphContainer) {
            ...
            // modify the css graph aspects
            htmlElement.style.width = `${
              htmlElement.getBoundingClientRect().width * scale
            }px`;

            htmlElement.style.height = `${
              htmlElement.getBoundingClientRect().height * scale
            }px`;
          }
        });
      }
